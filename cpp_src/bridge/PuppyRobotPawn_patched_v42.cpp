...

{
    ...
    CameraBoom->SocketOffset = FVector(0.0f, 80.0f, 220.0f);
...

{
...

    {
        ...
        constexpr double  INIT_X_M  = 0.05;   // scene_home.robot.initial_pose.x

        constexpr double  INIT_Y_M  = -2.98;   // scene_home.robot.initial_pose.y
...
static constexpr float HOME_BOUND_XMIN_CM = -561.0f;   // -4.88 m (=-4.90 + 0.02)

static constexpr float HOME_BOUND_XMAX_CM =  561.0f;   //  4.88 m (= 4.90 - 0.02)

static constexpr float HOME_BOUND_YMIN_CM = -446.0f;   // -3.88 m (=-3.90 + 0.02)

static constexpr float HOME_BOUND_YMAX_CM =  446.0f;   //  3.88 m (= 3.90 - 0.02)
...

{
...

    {
        ...
        // 绉诲姩鍚庤繘鍏ヤ簡闅滅鐗? 绉诲姩鍓嶄笉鍦?鈫?鏂扮鎾? 鍥為€€!
        ...
        UE_LOG(LogTemp, Warning, TEXT("[PuppyRobotPawn] BBOX-COLLIDE #%d: obs=[%s] (%.0f,%.0f)鈫?%.0f,%.0f) 鈫?REVERT"),
